...
so that the rounded array [Round1(p1),Round2(p2)...,Roundn(pn)] sums 
to the given target. Each operation Roundi(pi) could be either Floor(pi) or Ceil(pi).
...
class Solution(object):
    def minimizeError(self, prices, target):
        ...
        if not (sumFloor <= target <= sumCeil):
            return "-1"
        ...
        target = target - sumFloor
        for i in range(int(target)):
            ans += 1 - dif[i]
        for i in range(int(target), n):
            ans += dif[i]
...
if __name__ == "__main__":
    test = Solution()
    print(test.minimizeError(prices=["0.700", "2.800", "4.900"], target=8))
    print(test.minimizeError(prices=["1.500", "2.500", "3.500"], target=10))
    print(test.minimizeError(prices=["1.500", "2.500", "3.500"], target=9))
